...
//    gp << "set terminal qt title 'Generated Data'\n";
    ...
    gp << "plot '-' with points ps 1 pt 7 title 'Raw data', \
                ...
                '-' with lines linecolor rgb 'green' dashtype 2 smooth acsplines title 'Gnuplot spline'\n";
    gp.send1d(data);
...
void splineFromFunction(int pointsCount, int elementsCount, float noiseLevel){
...
//        data[i] = Point(x, exp(x) + dist(gen)*noiseLevel);
    ...
    for(int i = 0; i < nodesCount; i++){
        nodes[i] = x;//data[i * nodesCount].first;
        x += step;
    }
